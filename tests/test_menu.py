...
def test_format_menu_text_right_aligns_theme_and_clock() -> None:
    text = format_menu_text(width=80, theme_label="Sepia", clock_text="14:37")

    assert text.startswith(" F1 Help  F2 Theme")
    assert text.endswith("Theme: Sepia 14:37")
    assert len(text) == 80


def test_format_menu_text_uses_compact_menu_when_width_is_limited() -> None:
    text = format_menu_text(width=45, theme_label="Dark", clock_text="01:02")
